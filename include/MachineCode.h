...
{
private:
    MachineInstruction *parent;//指向父指令
    int type;
    int val;           // value of immediate number
    int reg_no;        // register number寄存器编号
...
protected:
    ...
    int op;                              // Instruction opcode 指令的操作码
    ...
    void PrintCond();
    //定义一些机器指令的类型：二元运算，加载，存储，移动，跳转，比较，栈操作
    enum instType
    {
        BINARY,
        LOAD,
...
public:
    ...
    void addZone() { zone++; };
    std::vector<SymbolEntry *> getGlobList() { return global_list; };
};
